...
export default function House() {
  ...
  const { owner, sports = [], rankings = [], best_at_everything = [], jack_of_all_trades = [], signature_sport = [] } = data;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Best at everything — only meaningful for ≥2-sport players */}
      {best_at_everything.length > 0 && (
        <Board title="🏆 Best at Everything" blurb="No weak spots — ranked by win % in each player's weakest sport.">
          <ol className="space-y-1 font-ui">
            {best_at_everything.map((r, i) => (
              <li key={r.user_id} className="flex items-center justify-between py-1.5 px-3 rounded-lg" style={{ background: 'var(--color-surface)' }}>
                ...
                <span style={{ color: 'var(--color-text-secondary)' }}>weakest {sportLabel(r.weakest_sport)} {r.min_win_pct}%</span>
              </li>
            ))}
          </ol>
        </Board>
      ...
      {/* Jack of all trades — breadth: winning records across multiple sports */}
      {jack_of_all_trades.length > 0 && (
        <Board title="🃏 Jack of All Trades" blurb="Winning records across the most sports (2+ sports).">
          <ol className="space-y-1 font-ui">
            {jack_of_all_trades.map((r, i) => (
              <li key={r.user_id} className="flex items-center justify-between py-1.5 px-3 rounded-lg" style={{ background: 'var(--color-surface)' }}>
                <span style={{ color: 'var(--color-text-primary)' }}>{i + 1}. {playerName(r)}<Nick p={r} /></span>
                <span style={{ color: 'var(--color-text-secondary)' }}>winning in {r.winning_sports} of {r.sports_played} sports</span>
              </li>
            ))}
          </ol>
        </Board>
      ...
      {/* Signature sport — each player's single strongest sport */}
      {signature_sport.length > 0 && (
        <Board title="🎯 Signature Sport" blurb="Each player's strongest sport by win rate (2+ sports).">
          <ol className="space-y-1 font-ui">
            {signature_sport.map((r) => (
              <li key={r.user_id} className="flex items-center justify-between py-1.5 px-3 rounded-lg" style={{ background: 'var(--color-surface)' }}>
                <span style={{ color: 'var(--color-text-primary)' }}>{playerName(r)}<Nick p={r} /></span>
                <span style={{ color: 'var(--color-text-secondary)' }}>{sportLabel(r.sport)} · {r.win_pct}%</span>
              </li>
            ))}
          </ol>
        </Board>
