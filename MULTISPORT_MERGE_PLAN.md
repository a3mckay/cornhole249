...
- [~] **Phase 4 — Cross-sport stats.** **Shipped (foundation, ahead of the next sport so it's "ready to go"):** pure DB-free aggregator `server/lib/house.js` (per-sport percentile math, average-rank ties, opponent ledger) + `buildOverview`/`buildH2H`/`buildNemesis`; HTTP layer `server/routes/house.js` mounted at `/api/house` (`/:ownerId/overview`, `/:ownerId/h2h/:p1/:p2`, `/:ownerId/nemesis/:userId`), house = leagues where `league_memberships.role='owner'`, official games only; client landing `client/src/pages/House.jsx` at `/house` + `/house/:ownerId` (rankings, best-at-everything, jack-of-all-trades, sport affinity), sport-agnostic (renders whatever sports the API reports). Tests: `houseLib.test.js` (pure math) + `house.test.js` (HTTP); 97/97 green. **Still open:** nav surfacing/house-landing placement (Open Item #3 — needs Andrew); cross-sport H2H + nemesis have endpoints but no dedicated client UI yet (overview page covers the boards); wiring achievements vs. board (Open Item #4).
